...
> renumber.** 6, 13 and 17 were deleted and their numbers are permanently retired. Only the ones
> below are always-on; the rest live in `.claude/rules/` (index follows) with their numbers intact.
...
42. **A gate whose predicate is computed OVER A SET passes vacuously when that set is empty — it reports "verified" having checked nothing, and that pass is indistinguishable from a real one.** `not (missing or duplicated or extra or reordered)` is trivially true when there is nothing that could be missing or duplicated, so a summarize rewrite of a file with no protected blocks satisfies the structure gate on ANY output, including the chat greeting a model returns when it answered the file instead of compressing it. Nothing errors and nothing warns, which is why this survives review. **The population that cannot satisfy a gate's premise is usually the one that most needs the check** — a file with no structure to protect is precisely the file that is all prose, and all prose is what a rewriter can be talked into responding to. **Require an AFFIRMATIVE signal that verification happened, and fail closed when there is none:** make the output carry something only a compliant run produces (`wrap.SOURCE_TAG` with its per-call nonce, echoed back), and treat "nothing to verify" as a refusal rather than a pass. Where a second cheap invariant exists, band it (`MIN_PROSE_FRACTION_OF_TARGET`) so a response that satisfies the format while saying nothing is still caught; prefer a one-sided band, since a false refusal costs a retry and a false pass overwrites the user's file. This is the sibling of a record that fails a filter's FORMAT and is therefore invisible to it rather than rejected by it: there the record escapes the filter, here the filter escapes having anything to check, and in both the absence of a complaint gets read as a verdict. Audit any predicate shaped like "no failures found" by asking what it returns on an empty collection.
