...
45. **A broad `except Exception` that means "log it and carry on" must ask whether the error was FATAL before it carries on — a DuckDB `FatalException` invalidates the whole database instance, not the connection that raised it.** Every handler written to absorb a per-record failure ("one row lost, use the in-memory view", "never crash a background thread") is correct for the errors it was written for and catastrophic for this one, and the difference is invisible at the catch site. After a fatal there are no more rows to skip, only queries that will all fail — so the process keeps serving traffic on a database it can no longer read, and the only symptom is a per-record warning. Call `core/db.handle_if_fatal` (or `is_fatal_db_error`) first, and let the ordinary logging run only when it answers no. **Three consequences that are not guessable and must not be re-derived by experiment each time:** (a) the invalidation is per-DATABASE, so a background job's "own" `DuckDBBackend` is not isolation — it shares the process's instance with the request path; (b) `duckdb.connect(path)` while ANY handle survives returns the SAME dead instance from DuckDB's per-path cache, so reconnecting without closing recovers nothing — every connection must be closed first, after which recovery works in-process with no restart; (c) fire-and-forget jobs swallow their own exceptions on the thread they spawn, so a guard placed around the DISPATCH sees nothing — put the classification at the handler that actually catches. **Corollary for surfaces:** a liveness probe that never touches storage cannot distinguish "serving" from "up but unable to read anything", so `/health` must query the database and report unhealthy rather than green.

46. **An index-integrity probe must compare against a form the index CANNOT serve, and `CAST(col AS VARCHAR)` is not that form for a column already stored as VARCHAR.** The cast is a no-op the planner discards, so both sides of the comparison get served by the very index under test and the probe reports a damaged index as sound — it is comparing the index against itself. Use `CAST(col AS VARCHAR) || ''`, which is a real expression over the column for every type; a `GROUP BY` aggregate is the independent third opinion when you need to be sure. Generalise past indexes: **whenever a check works by asking the same question two ways, prove the two ways actually take different paths before trusting agreement between them** — agreement is otherwise the expected result whether the system is healthy or broken, which is the failure mode of a check that has never once fired. Damaged DuckDB ART indexes make reads silently under-report as well as making writes fatal, so a blind probe here hides wrong analyzer numbers, not just a pending crash.
